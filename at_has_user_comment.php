...
function at_has_user_comment($user_id, $point_id) {
    ...
    $favorites = mysqli_query($db, "SELECT * FROM opinions WHERE user_id= '$user_id' AND point_id='$point_id'");
    if(!$favorites)
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['target']) && $_POST['target'] == 'external')
    if(isset($_POST['user_id']) && isset($_POST['point_id']))
        echo json_encode(at_has_user_comment($_POST['user_id'], $_POST['point_id']));
